...
if args.net=='res18':
    ...
    #from mlp import MLP
    from mlpMasked1LayerFix import MLP
    net = MLP(
    input_dim = 3072,
    hidden_dim = 3072,
    output_dim = 10,
...
#Making the change to load weights for fc1
#Load the weights for fc1
#pdb.set_trace() 
fc1_weights = torch.load('/p/dataset/abhishek/mlpmixerdense11_1-4-ckpt.t7', map_location=torch.device(device))['model']['fc1.weight']
# Set the weights of fc1 to the pre-calculated weights
net.fc1.weight.data = fc1_weights
net.fc1.weight.requires_grad = False
net.fc1.bias.requires_grad = False

# Train only the recurrent_layer1
#optimizer = torch.optim.SGD(net.recurrent_layer1.parameters(), lr=args.lr)
optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, net.recurrent_layer1.parameters()), lr=args.lr)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, '/p/dataset/abhishek/'+args.net+args.expName+str(args.expNum)+str(args.num_layers)+'_'+str(len(args.diagPos))+'-{}-ckpt.t7'.format(args.patch))
        best_acc = acc
