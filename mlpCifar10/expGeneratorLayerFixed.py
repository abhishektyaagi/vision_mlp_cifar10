...
def generate_bash_script(experiment_name,sparsity, num_layers):
    ...
    #pdb.set_trace()
    file_name = f'run_experimentsLayerFixed_{sparsity}_{num_diagonals1}_{num_layers}.sh'
    expNum = 1
    ...
    with open(file_name, 'w') as bash_script:
        ...
        for combination1 in combinations1:
            ...
            #cuda_visible_devices = i // 20 % 2
            #pdb.set_trace()
            cuda_visible_devices = 1
            # Construct the command string
            command = f"CUDA_VISIBLE_DEVICES={cuda_visible_devices} python train_cifar10LayerFixed.py --expName {experiment_name} --net mlpmixer --n_epochs 300 --lr 1e-3 --expNum {expNum} --sparsity {sparsity} --num_layers {num_layers} --diagPos {' '.join(map(str, combination1))} \n"
